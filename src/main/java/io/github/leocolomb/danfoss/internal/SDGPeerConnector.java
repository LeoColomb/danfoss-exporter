...
public class SDGPeerConnector {
    ...
    public void initialize(String peerIdStr) {
        ...
        if (peerId == null) {
            thingHandler.reportStatus("OFFLINE", "OFFLINE.CONFIGURATION_ERROR",
                    "Peer ID is not set");
            return;
        }
        ...
        watchdog = scheduler.scheduleAtFixedRate(() -> {
            ...
            if (System.currentTimeMillis() - lastPacket > 30000) {
                ...
                thingHandler.reportStatus("OFFLINE", "COMMUNICATION_ERROR",
    ...
    public void setOfflineStatus(Throwable t) {
        ...
        if (connection != null) {
            thingHandler.reportStatus("OFFLINE", "COMMUNICATION_ERROR", reason);
            scheduleReconnect();
        }
    }
    ...
    public void sendRefresh(int msgClass, int msgCode) {
        SendPacket(new Dominion.Packet(msgClass, msgCode));
    }

    private void reportStatus(@NonNull String status) {
        thingHandler.reportStatus(status, "NONE", null);
    }
}
